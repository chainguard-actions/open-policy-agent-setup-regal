  ...
  "node_modules/@actions/tool-cache/lib/tool-cache.js"(exports2) {
    ...
    function cacheDir(sourceDir, tool, version2, arch2) {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        core2.debug(`Caching tool ${tool} ${version2} ${arch2}`);
    ...
    function _createToolPath(tool, version2, arch2) {
      return __awaiter(this, void 0, void 0, function* () {
        const folderPath = path2.join(_getCacheDirectory(), tool, semver2.clean(version2) || version2, arch2 || "");
...
function getDownloadObject(version2) {
  const platform2 = os.platform();
  core.info(`Determining release asset for Regal ${version2} on ${platform2} ${os.arch()}`);
  const filename = `regal_${mapOS(platform2)}_${mapArch(os.arch())}`;
  const binaryName = platform2 === "win32" ? `${filename}.exe` : filename;
  const url = `https://github.com/open-policy-agent/regal/releases/download/v${version2}/${binaryName}`;
...
async function getVersion() {
  ...
  if (semver.valid(version2)) {
    return semver.clean(version2) || version2;
  }
  if (semver.validRange(version2)) {
    const max = semver.maxSatisfying(await getAllVersions(), version2);
    if (max) {
      return semver.clean(max) || version2;
    }
    core.warning(`${version2} did not match any release version.`);
  } else {
    core.warning(`${version2} is not a valid version or range.`);
  }
  return version2;
}
...
async function setup() {
  try {
    ...
    core.info(`Installing Regal ${version2}`);
    const download = getDownloadObject(version2);
    ...
    core.info(`Setup Regal ${version2}`);
  } catch (e) {
    core.setFailed(e);
  }
}
